...
<div class='warmWrap' id="tips">
	<img src="<%=path%>/statics/images/closeImg.gif" class='closeImg'
		onclick="closeTips()" />
	<p class='warminfos'>${COMPANY_FOR
		}支付平台提醒您：在购买商品和支付时，请注意核对网址，收款商户、商品名称、支付金额等信息，警惕各种形式的网络欺骗行为！</p>
</div>
...
<div class="containerGagteway auto">
	...
	<div class="pageContent">
		<form action="balancePay_balancePay.action" id="infoForm" class="form"
			method="post">
			...
			<div class="orderContent">
				<ul class="block" style="margin-top: 20px;">
					...
					<li>
						<div class="orderInfo">
							<div class="ada-tips">
								温馨提示：使用会员余额支付，直接扣减您${COMPANY_FOR
								}账户的余额，即可完成支付&nbsp&nbsp&nbsp&nbsp<a
									href="balancePay_changeBalanceAccount.action">使用其他账户支付</a>
							</div>
							</br>
							...
							<c:if test="${balance<sessionScope.PaymentOrderVo.orderAmount}">
								<div>
									<ul>
										...
										<li><span class="markRed flLt">您当前的账户余额不足，请登录门户后台充值后再支付&nbsp&nbsp您还可以通过选择
												<a href="javascript:history.go(-1);">网银支付</a>完成支付 ，有卡就能付。</span> <!--class="REbtn  ml5"  -->
										</li>
									</ul>
								</div>
							</c:if>
								...
								<table class="rest" cellpadding="0" cellspacing="0"
									...
									<tr>
										...
										<td><c:choose>
											...
											id="errorMsg" style="color:red;">${msg }</span>
										</td>
									</tr>
									<tr>
										<th class="left_td">
										...
										<td>
											...
											<div id="payDivId"
												...
												<span class="commonBtn" style=" margin-left: 20px;"
													id="paySpanId"> <span class="btn_lfRed"> <input
														id="Button1" class="btn_rtRed" type="button"
														onClick="submitChck()" value="确认支付" /> </span> </span>
											</div></td>
									</tr>
								</table>
							</c:if>
