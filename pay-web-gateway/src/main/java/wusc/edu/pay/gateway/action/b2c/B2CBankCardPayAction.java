 ...
 */
@Scope("prototype")
public class B2CBankCardPayAction extends GateWayAction {

	private static final long serialVersionUID = 3550648555487612073L;

	private static final Log log = LogFactory.getLog(B2CBankCardPayAction.class);
	@Autowired
	...
	public String execute() {
		...
		/** 调用交易限制 ---start **/
		tradeLimitFacade.validateTradeAmount(LimitTrxTypeEnum.PAY, payProductCode, frpCode, null, new BigDecimal(vo.getOrderAmount()), vo.getMerchantNo());
		...
		paymentRecord.setPayerAccountType(null);
		...
		log.info("商户编号:" + paymentRecord.getMerchantNo() + " b2c关网支付,支付流水号:" + paymentRecord.getTrxNo() + " [支付产品编号:" + paymentRecord.getPayProductCode() + "][支付方式 ：" + paymentRecord.getPayWayCode() + "] [银行接口:" + paymentRecord.getPayInterfaceCode() + "]" 
					+ "[付款方手续费: " + paymentRecord.getPayerAmountFee() + "]" + " 银行代码 bankCode:" + bankCode);
