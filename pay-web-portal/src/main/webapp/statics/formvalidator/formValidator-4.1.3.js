			...
			case "file":
				...
				if (initConfig.wideWord)
				{
					for (var i = 0; i < val.length; i++) 
					{
						...
						// 2013-12-18，改成一个中文也当一个长度，by吴水成
						len = len + ((val.charCodeAt(i) >= 0x4e00 && val.charCodeAt(i) <= 0x9fa5) ? 1 : 1); 
					}
				}
				else{
					len = val.length;
	...
	serialize : function(objs,initConfig)
	{
		...
		$.each(parmArray,function(index,data){
			...
			if(li_pos >0){
				...
				var value =encodeURI(encodeURI(decodeURIComponent(data.substr(li_pos+1))));
				var parm = name+"="+value;
	...
	reloadAutoTip : function(validatorGroup)
	{
		...
		$.each(initConfig.validObjects,function()
		{
			...
			if(initConfig.mode == "AutoTip" && setting.bind== true)
			{
				...
				//console.info($("#"+setting.tipID).parent());
				$(relativeID ).parent().append($("#"+setting.tipID));
				$(relativeID ).css({
					"display":"block",
					"float":"left"
				});
				$("#"+setting.tipID).css("float","left");
	...
	return this.each(function(e)
	{
		...
		{			
			...
			{		
				...
				tipDiv = $("<div style='float:left' id='"+tip+"'></div>").appendTo($("#"+relativeID ).parent());
				$("#"+relativeID ).css({
					"display":"block",
					"float":"left"
				});
