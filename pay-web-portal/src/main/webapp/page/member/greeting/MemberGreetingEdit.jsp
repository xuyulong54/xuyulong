...
function validatorFrom(){
	...
	// 预留信息
	$("#greeting").formValidator({onShow : "",onFocus : "必填项，长度为5-20个字符"})
	.inputValidator({min : 5,max : 20,empty : {leftEmpty : false,rightEmpty : false,emptyError : "输入内容两边不能有空符号"},onError : "不能为空，长度为5-20个字符"})
...
<div class="container">
	<div class="bd-container">
		<div class="frm-comon">
	<form action="memReservationInfo_editMemberGreeting.action" method="post" id="form1">
		<div class="ada-memberinfo">
			...
			<c:if test="${not empty  currentUser.greeting}">
			 <p class="clearfix">
				<label>原预留信息：</label>
				<span>${currentUser.greeting}</span>
				<input type="hidden" value="${currentUser.greeting}" name="oldGreeting" id="oldGreeting">
			</p>
			</c:if>
		 	<p class="clearfix">
				<label >新预留信息：</label>
				<input type="text" class="ada-input"value="${greeting}" name="greeting" id="greeting" />
			</p>
