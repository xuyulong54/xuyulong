...
<head>
	...
	$(document).ready(function() {
		setPageType('.men-pay', '.men-pay-transfer '); })
	//时间控制
...
<body>
    ...
    <div class="container">
	    <div class="bd-container">
	      <form action="memberPayTransfer_listTransfer.action" method="post" id="form">
	          ...
	          <a  id="exportExecl" href="memberPayTransfer_transferExportExcel.action?beginDate=${beginDate }&endDate=${endDate }&trxNo=${trxNo}" onclick="return exportExcel();">导出EXCEL
	          </a>
	        </div>
	      </form>
	      <div class="search_list">
					  ...
					  <thead>
						...
						<s:iterator value="recordList" status="st">
							<tr>
								...
								<td class='text-warning'><fmt:formatNumber value="${amount}" pattern="#0.00"></fmt:formatNumber></td>
								<td>
					...
					<div class="pageCla">
						<z:page pageBean="${pageBean }" url="memberPayTransfer_listTransfer.action" currentPage="${pageNum }"
							parameter="&beginDate=${beginDate }&endDate=${endDate }&sttype=${sttype}&trxNo=${trxNo}&timeType=${timeType}"></z:page>
					</div>
					</c:if>
					<br style="clear: both;" />
			  </div>
		...
		<table class="resaullist_head">
			...
			<p class="clearfix">
				<strong>交易流水号：</strong>
					...
					<input type="text" name="trxNo" value="${trxNo}">
			...
			<strong> &nbsp; </strong>
				...
				<a  id="exportExecl" href="memberPayTransfer_transferExportExcel.action?beginDate=${beginDate }&endDate=${endDate }&trxNo=${trxNo}" onclick="return exportExcel();" class="exportBtn">导出EXCEL</a>
			</p>
		</table>
		<div class="clear"></div>
		</div>
...
<div class="h10"></div>
	<div class="search_list">
		...
		<div class="pageCla">
			<z:page pageBean="${pageBean }" url="memberPayTransfer_listTransfer.action" currentPage="${pageNum }"
				parameter="&beginDate=${beginDate }&endDate=${endDate }&sttype=${sttype}&trxNo=${trxNo}"></z:page>
		</div>
		</c:if>
		<br style="clear: both;"/>
	    </div>
