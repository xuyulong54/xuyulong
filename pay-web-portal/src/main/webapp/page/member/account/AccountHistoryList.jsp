...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			setPageType('.men-account', '.men-account-search ');
		})
		//时间控制
...
<body onload="getNowDay()">
	...
	<div class="container">
		<div class="bd-container">
			<form action="memberaccount_listAccountHistory.action" method="post"
				id="form">
				<div class="query-head mtop10">
					<div class="title">账户明细查询</div>
				</div>
	              ...
	              <div class="select_cont">
	                <select name="trxType" class='select input-w170'>
										...
										<c:forEach items="${trxTypeList }" var="trxTypeEnum">
											<option value="${trxTypeEnum.value }" <c:if test="${trxTypeEnum.value ==trxType}">selected="selected"</c:if>>${trxTypeEnum.desc}</option>
										</c:forEach>
									</select>
	              </div>
	            </div>
						...
						<div class="frm-group">
					    <label>&nbsp;</label>
							<input id="Button1" class="btn btn-primary" type="button" value="查 询" onclick="checkData('form');" />
						</div>
						<div class="clear"></div>
					</div>
	      </div>
	      ...
	      <a id="exButton"
					href="memberaccount_exportExcel.action?beginDate=${beginDate }&endDate=${endDate}&fundDirection=${fundDirection}&trxType=${trxType}&requestNo=${requestNo}"
					...
					<tbody>
						<s:iterator value="recordList" status="st">
							<tr>
								...
								<td>${requestNo}</td>
								<td class='text-warning'>${fundDirection==123?'+':'-'}<fmt:formatNumber value="${amount}" pattern="#0.00" /></td>
								<td class='text-success'><fmt:formatNumber value="${balance}" pattern="#0.00" />
								</td>
								<td><c:forEach items="${trxTypeList}" var="trxTypeMap">
				...
				<c:if test="${pageBean.totalCount>0}">
					<div class="pageCla">
						<z:page pageBean="${pageBean }"
							url="memberaccount_listAccountHistory.action"
							currentPage="${pageNum }"
							parameter="&beginDate=${beginDate }&endDate=${endDate}&fundDirection=${fundDirection}&requestNo=${requestNo }&trxType=${trxType}&timeType=${timeType}"></z:page>
					</div>
				</c:if>
				<br style="clear: both;" />
			</div>
