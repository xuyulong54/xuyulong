...
<div class="pageContent">
	<form id="form" method="post" action="bankBranch_doAdd.action" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
		<div class="pageFormContent" layoutH="60">
			...
			<div class="unit">
				<label>支付渠道编号：</label>
				<select name="frpCode" onchange="getFrpCode(this.value)" id="frpCode" onmousedown="return false" style="font-size:14px;">
			...
			<c:choose>
				<c:when test="${fn:contains(defaultBankChannelCode,frpCode)}">
				<input type="radio" checked="checked" onclick="loadSlect()"  name="frps"> 本行 
				<input type="radio" onclick="loadAll()" name="frps"> 其它银行
				</c:when>
				<c:otherwise>
				<input type="radio" onclick="loadSlect()"  name="frps"> 本行 
				<input type="radio" onclick="loadAll()" name="frps" checked="checked"> 其它银行
				</c:otherwise>
			</c:choose>
			</div>
			<div class="unit">
...
function loadAll(){
	...
	//是本行还是其它银行时 加载方法 如果是非本行，则调用下面方法
	$(function(){
		<c:if test="${fn:contains(defaultBankChannelCode,frpCode) == false}">
			loadAll();
		</c:if>
