...
<div class="pageContent">
	<form id="form" method="post" action="remitChannel_remitChannelUpdate.action" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
		...
		<div class="pageFormContent" layoutH="60" >
			...
			<p>
				<label>总笔数限制：</label>
				<input type="text" id="limitNum" value="${limitNum}"  class="digits required" min="1" max="1000"  size="30" name="limitNum" />
			</p>
			...
			<p>
				...
				<c:forEach items="${isOrNotEnumList }" var="isOrNotEnum" varStatus="index">
					...
					<c:if test="${isConfirm eq isOrNotEnum.value }">checked="checked"</c:if> 
					 />${isOrNotEnum.desc}
				</c:forEach>
			</p> --%>
			...
			<p>
				...
				<c:forEach items="${isOrNotEnumList }" var="isOrNotEnum" varStatus="index">
					...
					<c:if test="${isAutoRemit eq isOrNotEnum.value }">checked="checked"</c:if> 
					 />${isOrNotEnum.desc}
				</c:forEach>
			</p>
			...
			<p>
				...
				<c:forEach items="${bankAccountTypeEnumList }" var="bankAccountTypeEnum" varStatus="index">
					...
					<c:if test="${bankAccountTypeEnum.value eq accountType }">checked="checked"</c:if> 
